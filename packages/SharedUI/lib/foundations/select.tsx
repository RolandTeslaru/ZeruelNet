...
function SelectValue({
  ...
  return <SelectPrimitive.Value className="text-label-primary" data-slot="select-value" {...props} />
}
...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      className={cn(
        `flex w-full items-center justify-between rounded-md shadow-md shadow-black/20
    ...
    >
      ...
      <SelectPrimitive.Icon asChild>
        <ChevronDown className="h-4 w-4 opacity-50 text-label-primary" />
    ...
    >
      <span className="absolute left-2 flex h-3.5 w-3.5 items-center justify-center text-label-primary">
        <SelectPrimitive.ItemIndicator>
          <Check className="h-4 w-4 text-label-primary" />
        </SelectPrimitive.ItemIndicator>
      </span>
...
const SelectSeparator: FC<ComponentProps<typeof SelectPrimitive.Separator>> = ({ className, ...props }) => (
