import { ScrapedVideo } from "@zeruel/harvester-types";

const ScrapedVideoCard = ({ video }: { video: ScrapedVideo }) => {
    return (
        <div className="flex items-center p-3 bg-gray-800 rounded-lg shadow-md gap-4">
            ...
            <div className="flex-grow">
                <a href={video.video_url} target="_blank" rel="noopener noreferrer" className="hover:underline">
                    <p className="text-sm text-blue-400 truncate">{`ScrapedVideo ID: ${video.video_id}`}</p>
                </a>
...
//                 )}
//                 {scrapedVideos.map((video: ScrapedVideo) => (
