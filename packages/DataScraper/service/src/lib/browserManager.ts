...
const SOURCE_DATA_PATH_IN_CONTAINER = path.join(__dirname, '..', '..', 'tiktok_user_data');
...
export class BrowserManager {
    ...
    async init(): Promise<void> {
        ...
        // Railway needs some special handling because it needs to copy the tiktok user data 
        // into its own persistent volume on the first run
        if (isRailway)
            handleOnRailway()
        else
