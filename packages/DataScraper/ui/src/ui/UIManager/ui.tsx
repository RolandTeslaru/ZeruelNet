...
import { DialogType, useUIManagerAPI } from './store'
import { AlertDialog, AlertDialogContent, Dialog, DialogContent } from '../foundations';
...
const DIALOG_MAP = {
  normal: ({ id, content, open, closeDialog, className, index, dialogTotal, theme }: any) => {
  ...
  alert: ({ id, content, open, closeDialog, className, index, dialogTotal, showTriangle, type }: AlertDialogProps) => {
    const scale_offset = (index - (dialogTotal - 1)) * 8;
    const y_offset = (index - (dialogTotal - 1)) * 40;
    const finalScale = 1 + scale_offset / 100;
  ...
  danger: ({ id, content, open, closeDialog, className, index, dialogTotal, showTriangle, type }: AlertDialogProps) => {
    const scale_offset = (index - (dialogTotal - 1)) * 8;
    const y_offset = (index - (dialogTotal - 1)) * 40;
    const finalScale = 1 + scale_offset / 100;
